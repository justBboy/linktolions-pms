...
router.get("/chats/newCount/:id", isAuthenticated, async (req, res) => {
  const id = req.params.id;
  const lastRefreshDate = req.query.lastRefreshDate;
  try {
      let messagesCount;
      if(lastRefreshDate){
          ...
          messagesCount = await Message.count({receipient: id});
      }
